...
export class Game extends Phaser.Scene {
	...
	async update() {
		...
		// Posição absoluta do pássaro na tela: topo -1, centro 0, chão +1
		const normalizedBirdY = (this.bird.y / this.scale.height) * 2 - 1;
	...
	async hitPipe() {
		...
		if (this.lastState !== null && this.lastAction !== null) {
			...
			const terminalState = [0, 0, 0, 0, 0, 0, 0, 0, 0];
			this.agent.replayBuffer.add(
				this.lastState,
				this.lastAction,
				reward,
